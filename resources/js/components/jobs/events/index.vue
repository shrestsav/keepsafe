      ...
      <div class="nav-wrapper">
        <ul class="nav nav-pills nav-fill flex-column flex-md-row" id="tabs-icons-text" role="tablist">
          <li class="nav-item" v-for="status,key in eventStatuses">
            <a class="nav-link mb-sm-3 mb-md-0" :class="key=='1' ? 'active' : ''" :id="key" data-toggle="tab" href="" role="tab" aria-controls="tabs-icons-text-1" aria-selected="false" @click="getEvents(key)"><i :class="'fas fa-'+icons[key]+' mr-2'"></i>{{status}}</a>
          </li>
        </ul>
      </div>
    </div>
